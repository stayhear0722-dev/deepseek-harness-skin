...
describe('ConversationRoot resident composer', () => {
  it('raises the presentation glass state while the projected session is running', () => {
    const b = mount(conversationSnapshot({ running: true }))
    const root = b.view.container.querySelector('[data-phase]')
    expect(root?.hasAttribute('data-running')).toBe(true)
    expect(root?.querySelector('[aria-hidden="true"]')).toBeTruthy()

    b.session.set(conversationSnapshot({ running: false }))
    b.rerender()

    expect(root?.hasAttribute('data-running')).toBe(false)
  })
